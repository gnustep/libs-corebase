...
#define URL_IS_SCHEME(c)                                                       \
  (CHAR_IS_ALPHA(c) || CHAR_IS_DIGIT(c) || c == '+' || c == '-' || c == '.')
#define URL_IS_PCHAR(c)                                                        \
  (URL_IS_UNRESERVED(c) || c == '%' || URL_IS_SUB_DELIMS(c) || c == ':'        \
   || c == '@')

#define URL_IS_RESERVED(c) (URL_IS_GEN_DELIMS(c) || URL_IS_SUB_DELIMS(c))
#define URL_IS_UNRESERVED(c)                                                   \
  (CHAR_IS_ALPHA(c) || CHAR_IS_DIGIT(c) || c == '-' || c == '.' || c == '_'    \
   || c == '~')

#define URL_IS_GEN_DELIMS(c)                                                   \
  (c == ':' || c == '/' || c == '?' || c == '#' || c == '[' || c == ']'        \
   || c == '@')
#define URL_IS_SUB_DELIMS(c)                                                   \
  (c == '!' || c == '$' || c == '&' || c == '\'' || c == '(' || c == '*'       \
   || c == '+' || c == ',' || c == ';' || c == '=')
...
CF_INLINE Boolean CFURLIsDecomposable(CFURLRef url) {
  return ((CFRuntimeBase *) url)->_flags.info & _kCFURLIsDecomposable ? true : false;
}

CF_INLINE Boolean CFURLIsFileSystemPath(CFURLRef url)
{
  return ((CFRuntimeBase *) url)->_flags.info & _kCFURLIsFileSystemPath ? true
                  : false;
}

CF_INLINE void CFURLSetIsDecomposable(CFURLRef url) {
  ((CFRuntimeBase *) url)->_flags.info |= _kCFURLIsDecomposable;
}

CF_INLINE void CFURLSetIsFileSystemPath(CFURLRef url) {
  ((CFRuntimeBase *) url)->_flags.info |= _kCFURLIsFileSystemPath;
}

static void CFURLFinalize(CFTypeRef cf) {
...
static Boolean CFURLEqual(CFTypeRef cf1, CFTypeRef cf2) {
    Boolean  equal;
...
static CFHashCode CFURLHash(CFTypeRef cf) {
  CFHashCode hash;
...
static Boolean CFURLStringParse(CFStringRef urlString, CFRange ranges[12]) {
    ...
    */
    ...
    if (CHAR_IS_ALPHA(c)) {
        ...
        if (c == ':') {
            ranges[kCFURLComponentScheme - 1] = CFRangeMake(start, idx - start - 1);
            hasScheme = true;
        } else {
            idx = 0; /* Start over... */
        }
    ...
    */
        ...
        while (i < end) {
            ...
            */
            ...
            if (c == '@') {
                ...
                ranges[kCFURLComponentUserInfo - 1] = CFRangeMake(start, i - start);
                userInfoEnd = i;
                ...
                while (i < userInfoEnd) {
                    c = CFStringGetCharacterFromInlineBuffer(&iBuffer, i++);
                    if (c == ':') {
                        ranges[kCFURLComponentPassword - 1] = CFRangeMake(i, userInfoEnd - i - 1);
                        break;
                    }
                }

                ranges[kCFURLComponentUser - 1] = CFRangeMake(start, i - start - 1);
                start = userInfoEnd;
                break;
            }
        }
    ...
    if (c == ';') {
        ...
        ranges[kCFURLComponentParameterString - 1] = CFRangeMake(start, idx - start - 1);
    ...
    if (resourceSpecifierStart != kCFNotFound) {
        ranges[kCFURLComponentResourceSpecifier - 1] = CFRangeMake(resourceSpecifierStart, idx - resourceSpecifierStart);
    }
...
static void CFURLStringAppendByRemovingDotSegments(CFMutableStringRef string, UniChar *buffer, CFIndex length) {
    UniChar *bufferStart;
    UniChar *bufferEnd;
    CFIndex  pathStart;
    ...
    if (buffer < bufferEnd && *buffer == '.') {
        if ((buffer + 1) < bufferEnd && buffer[1] == '/') {
            buffer += 1;
        } else if ((buffer + 2) < bufferEnd && buffer[1] == '.' && buffer[2] == '/') {
            buffer += 2;
        }
    }
    bufferStart = buffer;
    ...
    while (buffer < bufferEnd) {
        if ((buffer + 1) < bufferEnd && buffer[0] == '/' && buffer[1] == '.') {
            /* Skip '/./' or '/.'EOS */
            ...
            || ((buffer + 3) == bufferEnd && buffer[2] == '.')) {
                ...
                CFStringDelete(string, CFRangeMake(pathStart + i + 1, pathLength - i - 1));
...
CFURLRef CFURLCopyAbsoluteURL(CFURLRef relativeURL)
{
    ...
    */
    ...
    if (range.location != kCFNotFound) {
        ...
        if (range.location != kCFNotFound) {
            CFStringGetCharacters(relString, range, buffer);
            CFURLStringAppendByRemovingDotSegments(targetString, buffer, range.length);
        }
            ...
            if (range.location != kCFNotFound) {
                ...
                if (range.length == 0) {
                    ...
                    if (CFStringGetCharacterAtIndex(relString, range.location) == '/') {
                        ...
                        CFStringGetCharacters(relString, range, &buffer[baseRange.length]);
    ...
    target = CFURLCreate_internal(alloc, targetString, NULL, kCFStringEncodingUTF8);
...
CFURLRef CFURLCreateAbsoluteURLWithBytes(CFAllocatorRef alloc, const UInt8 *relativeURLBytes, CFIndex length, CFStringEncoding encoding, CFURLRef baseURL, Boolean useCompatibilityMode) {
    ...
    str = CFStringCreateWithBytes(alloc, relativeURLBytes, length, encoding, false);
    if (str == NULL) {
        return NULL;
    }
...
CFURLRef CFURLCreateFromFileSystemRepresentation(CFAllocatorRef alloc, const UInt8 *buffer, CFIndex bufLen, Boolean isDirectory) {
    return CFURLCreateFromFileSystemRepresentationRelativeToBase(alloc, buffer, bufLen, isDirectory, NULL);
}
...
CFURLRef CFURLCreateFromFileSystemRepresentationRelativeToBase(CFAllocatorRef alloc, const UInt8 *buffer, CFIndex bufLen, Boolean isDirectory, CFURLRef baseURL) {
    ...
    path = CFStringCreateWithBytesNoCopy(alloc, buffer, bufLen, GSStringGetFileSystemEncoding(), false, kCFAllocatorNull);
    ret = CFURLCreateWithFileSystemPathRelativeToBase(alloc, path, CFURL_DEFAULT_PATH_STYLE, isDirectory, baseURL);
...
CFURLRef CFURLCreateWithFileSystemPath(CFAllocatorRef alloc, CFStringRef fileSystemPath, CFURLPathStyle style, Boolean isDirectory) {
    CFURLRef ret;

    ret = CFURLCreateWithFileSystemPathRelativeToBase(alloc, fileSystemPath, style, isDirectory, NULL);
...
CF_INLINE CFURLRef CFURLCreateWithCurrentDirectory(CFAllocatorRef alloc) {
    CFURLRef ret;
    CFStringRef cwd;
    ...
    cwd = CFStringCreateWithBytesNoCopy(alloc, (const UInt8 *)buffer, length, GSStringGetFileSystemEncoding(), false, kCFAllocatorNull);
#else
    char buffer[1024];
    ...
    cwd = CFStringCreateWithBytesNoCopy(alloc, (const UInt8 *) buffer, strlen(buffer), GSStringGetFileSystemEncoding(), false, kCFAllocatorNull);
#endif
...
static CFStringRef CFURLCreateStringFromHFSPathStyle(CFAllocatorRef alloc, CFStringRef filePath, Boolean isAbsolute, Boolean isDirectory) {
    return NULL; /* FIXME */
}

static CFStringRef CFURLCreateStringFromWindowsPathStyle(CFAllocatorRef alloc, CFStringRef filePath, Boolean isAbsolute, Boolean isDirectory) {
    ...
    while (idx < count) {
      ...
      escaped = CFURLCreateStringByAddingPercentEscapes(alloc, unescaped, NULL, NULL, kCFStringEncodingUTF8);
...
CFURLRef CFURLCreateWithFileSystemPathRelativeToBase(CFAllocatorRef alloc, CFStringRef filePath, CFURLPathStyle style, Boolean isDirectory, CFURLRef baseURL) {
    ...
    switch (style) {
        case kCFURLPOSIXPathStyle:
            ...
            path = CFURLCreateStringByAddingPercentEscapes(alloc, filePath, NULL, NULL, kCFStringEncodingUTF8);
            if (path != filePath)
                CFRetain(path);
            filePathLen = CFStringGetLength(path);
            if (isDirectory && CFStringGetCharacterAtIndex(path, filePathLen) != '/') {
                CFStringRef tmp;
        ...
        case kCFURLHFSPathStyle:
            ...
            */
            abs = (CFStringGetCharacterAtIndex(filePath, 0) == ':');
            path = CFURLCreateStringFromHFSPathStyle(alloc, filePath, abs, isDirectory);
        break;
        case kCFURLWindowsPathStyle:
            abs = (CFStringGetCharacterAtIndex(filePath, 1) == ':' && 
                   CFStringGetCharacterAtIndex(filePath, 2) == '\\');
            path = CFURLCreateStringFromWindowsPathStyle(alloc, filePath, abs, isDirectory);
            break;
        default:
            return NULL;
    }
...
CFURLRef CFURLCreateWithBytes(CFAllocatorRef alloc, const UInt8 *bytes, CFIndex length, CFStringEncoding encoding, CFURLRef baseURL) {
    ...
    str = CFStringCreateWithBytesNoCopy(NULL, bytes, length, encoding, false, kCFAllocatorNull);
...
static CFStringRef CFURLCreateHFSStylePath(CFAllocatorRef alloc, CFStringRef path) {
    return NULL;
}

static CFStringRef CFURLCreateWindowsStylePath(CFAllocatorRef alloc, CFStringRef path) {
    CFArrayRef  comps;
    CFStringRef ret;
...
CFStringRef CFURLCopyFileSystemPath(CFURLRef url, CFURLPathStyle style) {
    ...
    if (r.length > 1) {
        ...
        path = CFURLCreateStringByReplacingPercentEscapesUsingEncoding(alloc, tmp, CFSTR(""), url->_encoding);
        CFRelease(tmp);
    } else {
        path = CFSTR("/");
    }
...
CFStringRef CFURLCopyFragment(CFURLRef url, CFStringRef charactersToLeaveEscaped) {
    ...
    return CFStringCreateWithSubstring(CFGetAllocator(url), url->_urlString, range);
}
...
CFStringRef CFURLCopyStrictPath(CFURLRef url, Boolean *isAbsolute) {
    ...
    if (path) {
        if (CFStringGetCharacterAtIndex(path, 0) == '/') {
            ...
            abs = true;
            tmp = CFStringCreateWithSubstring(CFGetAllocator(url), path, CFRangeMake(1, CFStringGetLength(path) - 1));
            CFRelease(path);
            path = tmp;
        }
    }
...
SInt32 CFURLGetPortNumber(CFURLRef url) {
    ...
    str = CFStringCreateWithSubstring(CFGetAllocator(url), url->_urlString, range);
...
CFDataRef CFURLCreateData(CFAllocatorRef alloc, CFURLRef url, CFStringEncoding encoding, Boolean escapeWhiteSpace) {
    ...
    if (escapeWhiteSpace) {
        absStr = CFURLCreateStringByAddingPercentEscapes(alloc, absStr, NULL, CFSTR(" \r\n\t"), encoding);
    }
...
static Boolean CFURLAppendPercentEscapedForCharacter(char **dst, UniChar c, CFStringEncoding enc) {
    CFIndex len;
...
static Boolean CFURLStringContainsCharacter(CFStringRef toEscape, UniChar ch) {
    CFStringInlineBuffer iBuffer;
    CFIndex sLength;
    CFIndex i;
    UniChar c;
...
CF_INLINE Boolean CFURLShouldEscapeCharacter(UniChar c, CFStringRef leaveUnescaped, CFStringRef toEscape) {
...
CFStringRef CFURLCreateStringByAddingPercentEscapes(CFAllocatorRef alloc, CFStringRef origString, CFStringRef leaveUnescaped, CFStringRef toEscape, CFStringEncoding encoding) {
    CFStringInlineBuffer iBuffer;
    CFStringRef ret;
    CFIndex sLength;
    CFIndex idx;
    ...
    for (idx = 0; idx < sLength; ++idx) {
        ...
        if (CFURLShouldEscapeCharacter(c, leaveUnescaped, toEscape)) {
            if (dst == NULL) {
                ...
                CFStringGetBytes(origString, CFRangeMake(0, idx), kCFStringEncodingASCII, 0, false, (UInt8 *) dst, sLength * 3, NULL);
                dpos = dst + idx;
            }
    ...
    if (dst) {
        ret = CFStringCreateWithBytes(alloc, (UInt8 *) dst, (CFIndex) (dpos - dst), kCFStringEncodingASCII, false);
        CFAllocatorDeallocate(alloc, dst);
    } else {
...
CFStringRef CFURLCreateStringByReplacingPercentEscapes(CFAllocatorRef alloc, CFStringRef origString, CFStringRef leaveEscaped)
{
    return CFURLCreateStringByReplacingPercentEscapesUsingEncoding(alloc, origString, leaveEscaped, kCFStringEncodingUTF8);
}

CFStringRef CFURLCreateStringByReplacingPercentEscapesUsingEncoding(CFAllocatorRef alloc, CFStringRef origString, CFStringRef leaveEscaped, CFStringEncoding encoding)
{
    CFIndex origLen;
    CFRange origRange;
    CFRange escapeRange;
    ...
    while (success && CFStringFindWithOptions(origString, CFSTR("%"), origRange, 0, &escapeRange)) {
        CFIndex subLen;
        CFIndex i;
        UniChar ch;
        ...
        unescapedString = CFStringCreateWithBytes(kCFAllocatorSystemDefault, (const UInt8 *) buffer, current - buffer, encoding, false);
...
Boolean CFURLGetFileSystemRepresentation(CFURLRef url, Boolean resolveAgainstBase, UInt8 *buffer, CFIndex bufLen) {
    ...
    unescaped = CFURLCreateStringByReplacingPercentEscapesUsingEncoding(NULL, str, CFSTR(""), enc);
    CFRelease(str);
...
CFIndex CFURLGetBytes(CFURLRef url, UInt8 *buffer, CFIndex bufLen) {
    ...
    converted = CFStringGetBytes(str, CFRangeMake(0, length), enc, 0, false, buffer, bufLen, &used);
    if (converted != used) {
        used = -1;
    }
...
CFDictionaryRef CFURLCreateResourcePropertiesForKeysFromBookmarkData(CFAllocatorRef alloc, CFArrayRef resourcePropertiesToReturn, CFDataRef bookmark) {
    return NULL; /* FIXME */
}

CFTypeRef CFURLCreateResourcePropertyForKeyFromBookmarkData(CFAllocatorRef alloc, CFStringRef resourcePropertyKey, CFDataRef bookmark) {
    return NULL; /* FIXME */
}
...
CFDataRef CFURLCreateBookmarkDataFromFile(CFAllocatorRef alloc, CFURLRef fileURL, CFErrorRef *errorRef) {
    return NULL; /* FIXME */
}
